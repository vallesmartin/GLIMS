...
{
    public class ShipDeliveriesViewModel : BaseViewModel
    {
        ...
        public async Task OnAppear()
        {
            await Task.Run(async () => {
                // Obtiene observable
                Deliveries = new ObservableCollection<DocumentModel>(await Repository.Delivery_GetByStatusAsync(20));
                // Ordena y cuenta
                Deliveries = new ObservableCollection<DocumentModel>(Deliveries.OrderBy(d => d.DocumentStatus).ThenByDescending(d => d.DocumentDate).ToList());
